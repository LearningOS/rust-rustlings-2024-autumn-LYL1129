...
impl<T> LinkedList<T> {
        ...
        T: Clone + PartialOrd,
    {
        ...
        while a.is_some() && b.is_some() {
            unsafe {
                ...
                if a_val <= b_val {
                    ...
                    a = (*a.unwrap().as_ptr()).next; // Move to next node in list_a
                } else {
                    merged_list.add(b_val.clone());
                    b = (*b.unwrap().as_ptr()).next; // Move to next node in list_b
                }
            }
        }
        ...
        while a.is_some() {
            unsafe {
                merged_list.add((*a.unwrap().as_ptr()).val.clone());
        ...
        while b.is_some() {
            unsafe {
                merged_list.add((*b.unwrap().as_ptr()).val.clone());
    ...
    T: Display,
{
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        ...
        while let Some(node) = current {
            if !first {
                write!(f, " -> ")?; // Formatting for linked list
            }
